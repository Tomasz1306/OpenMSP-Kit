...
public class TMDBGenreDownloader implements Downloader<List<TMDBGenre>, String> {
    ...
    public List<TMDBGenre> fetch(String movieId) {
        List<TMDBMovieGenreResponse> genreResponses = new ArrayList<>();
    ...
    private List<TMDBMovieGenreResponse> download(String movieId, String language) {
        try {
            ...
            return response
                    .genres()
                    .stream()
                    .map(genre -> new TMDBMovieGenreResponse(genre.tmdbId(), genre.name())).toList();
        } catch (Exception e) {
